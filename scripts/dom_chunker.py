...
class HTMLChunker:
    ...
    def reassemble_html(self, chunks=None, content_field_name='content', cleanup=False):
        head_content = ''.join(chunk[content_field_name] for chunk in chunks if chunk['section'] == 'head')
        body_content = ''.join(chunk[content_field_name] for chunk in chunks if chunk['section'] == 'body')
        ...
        # remove data-chunk-uuid attributes
        head_content = re.sub(r'\s*data-chunk-uuid="[^"]*"', '', head_content)
        body_content = re.sub(r'\s*data-chunk-uuid="[^"]*"', '', body_content)

        if cleanup:
            # remove <!-- Optimized by LLM --> and <!-- End of Optimization: -->
            head_content = re.sub(r'<!--\s*Optimized by LLM\s*-->', '', head_content)
            body_content = re.sub(r'<!--\s*Optimized by LLM\s*-->', '', body_content)

            head_content = re.sub(r'<!--\s*End of Optimization:.*?-->', '', head_content, flags=re.DOTALL)
            body_content = re.sub(r'<!--\s*End of Optimization:.*?-->', '', body_content, flags=re.DOTALL)
    ...
    def replace_styles(self, content, style_store):
        ...
        for chunk_id, style_content in style_store.items():
            if style_content is None:
                content = content.replace(f'/* chunk_style_{chunk_id} */', '')
            else:
    ...
    def replace_scripts(self, content, script_store):
        ...
        for chunk_id, script_content in script_store.items():
            if script_content is None:
                content = content.replace(f'// chunk_script_{chunk_id}', '')
            else:
                content = content.replace(f'// chunk_script_{chunk_id}', script_content)
        return content

    def store_chunks(self, chunks, chunked_file_directory="./../dataset/chunks/", direct=False, store_chunks=True):
